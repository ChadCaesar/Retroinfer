...
class QwenModel(LLM):
    ...
    def __init__(
        ...
        self.config = Qwen2Config.from_pretrained(os.path.join(MODEL_ROOT, model_name.split("/")[-1]))
    ...
    def init_model(self):
        hf_qwen = Qwen2ForCausalLM.from_pretrained(os.path.join(MODEL_ROOT, self.model_name.split("/")[-1]), torch_dtype=self.dtype)
